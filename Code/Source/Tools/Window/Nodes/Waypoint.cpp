    ...
    namespace Nodes
    {
        void WaypointNode::Reflect(AZ::ReflectContext* context)
        {
            if (auto* serialize = azrtti_cast<AZ::SerializeContext*>(context))
            {
                ...
                {
                    editContext->Class<WaypointNode>("ActorDialogue", "Represents an actor's response.")
        ...
        auto WaypointNode::CanExtendSlot(GraphModel::SlotDefinitionPtr slotDefinition) const -> bool
        {
        ...
        {
            auto const dialogueIdDataType = GetGraphContext()->GetDataType(AZ_CRC_CE("dialogue_id"));
            auto input = AZStd::make_shared<GraphModel::SlotDefinition>(
                GraphModel::SlotDirection::Input, GraphModel::SlotType::Data, ToString(NodeAndSlotNames::Waypoint_In), "In", "",
            ...
            auto output = AZStd::make_shared<GraphModel::SlotDefinition>(
                GraphModel::SlotDirection::Output, GraphModel::SlotType::Data, ToString(NodeAndSlotNames::Waypoint_Out), "Out", "",
                GraphModel::DataTypeList{ dialogueIdDataType }, dialogueIdDataType->GetDefaultValue(), 1, 1);
        ...
        void WaypointNode::PostLoadSetup(GraphModel::GraphPtr graph, GraphModel::NodeId id)
        {
