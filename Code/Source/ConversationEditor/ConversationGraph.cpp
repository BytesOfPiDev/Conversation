...
namespace ConversationEditor
{
    ...
    GraphModel::NodePtr ConversationGraph::FindRoot()
    {
        ...
        auto iter = std::find_if(
            nodemap.begin(), nodemap.end(),
            [](AZStd::pair<GraphModel::NodeId, GraphModel::NodePtr> nodepair)
            {
    ...
    void ConversationGraph::ProcessLinkNode(
        AZStd::shared_ptr<Conversation::ConversationAsset> ConversationAsset, const GraphModel::NodePtr linkNode)
    {
        /**
         ...
         */
        GraphModel::Slot::ConnectionList enterConnectionList = linkNode->GetSlot(CommonSlotNames::ENTER)->GetConnections();
        ...
        if (!enterConnectionList.empty())
        {
            ...
            auto parentDialogueDataPtr = (*enterConnectionList.begin())
                                             ->GetSourceNode()
                                             ->GetSlot(CommonSlotNames::DIALOGUEDATA)
                                             ->GetValue<Conversation::DialogueDataPtr>();
            ...
            AZ_Error(
                "DialogueGraph", dialogueIdPropertyConnectionList.size() == 1,
            ...
            if (!dialogueIdPropertyConnectionList.empty())
            {
                 ...
                 * the request.
                 */
                ConversationAsset->AddResponseId(parentDialogueDataPtr->GetId(), responseDialogueDataPtr->GetId());
            }
        }
    }

    void ConversationGraph::ProcessDialogueNode(
        AZStd::shared_ptr<Conversation::ConversationAsset> ConversationAsset, const GraphModel::NodePtr actorNode)
    {
            ...
            for (auto slotConnection : slot->GetConnections())
            {
                ...
                if (responseActorDialogueNode->RTTI_IsTypeOf(ConversationEditor::Nodes::ActorDialogue::TYPEINFO_Uuid()))
                {
                    ...
                    AZ_TracePrintf(
                        ...
                        responseDialogueDataPtr->GetActorText().c_str());
                }
            }
        }
