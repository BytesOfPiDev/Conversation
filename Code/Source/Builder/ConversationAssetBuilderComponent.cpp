...
namespace ConversationEditor
{
    void DialogueAssetBuilderComponent::Reflect(AZ::ReflectContext* context)
    {
        ...
        {
            serializeContext
                ->Class<DialogueAssetBuilderComponent, AZ::Component>()
                ->Version(0)
                ->Attribute(
    ...
    void DialogueAssetBuilderComponent::Activate()
    {
        ...
        builderDescriptor.m_patterns.emplace_back(
            Conversation::ConversationAsset::ProductExtensionPattern,
            AssetBuilderSDK::AssetBuilderPattern::PatternType::Wildcard);
        builderDescriptor.m_patterns.emplace_back(
            Conversation::ConversationAsset::SourceExtensionPattern,
            AssetBuilderSDK::AssetBuilderPattern::PatternType::Wildcard);
        builderDescriptor.m_busId =
            azrtti_typeid<ConversationAssetBuilderWorker>();
        builderDescriptor.m_version =
            1; // if you change this, all assets will automatically rebuild
        builderDescriptor.m_analysisFingerprint =
            ""; // if you change this, all assets will re-analyze but not
                // necessarily rebuild.
        builderDescriptor.m_createJobFunction =
            [ObjectPtr = &m_dialogueAssetBuilder](auto&& PH1, auto&& PH2)
        {
            ObjectPtr->CreateJobs(
                AZStd::forward<decltype(PH1)>(PH1),
                AZStd::forward<decltype(PH2)>(PH2));
        };
        ...
        // note that this particular builder does in fact emit various kinds of
        // dependencies (as an example). if your builder is simple and emits no
        // dependencies (for example, it just processes a single file and that
        // file doesn't really depend on any other files or jobs), setting the
        // BF_EmitsNoDependencies flag will improve "fast analysis" scan
        // performance.
    ...
    void DialogueAssetBuilderComponent::GetProvidedServices(
        AZ::ComponentDescriptor::DependencyArrayType& provided)
    {
        provided.push_back(AZ_CRC_CE("ConversationAssetBuilderService"));
    }

    void DialogueAssetBuilderComponent::GetIncompatibleServices(
        AZ::ComponentDescriptor::DependencyArrayType& incompatible)
    {
        incompatible.push_back(AZ_CRC_CE("ConversationAssetBuilderService"));
    }

    void DialogueAssetBuilderComponent::GetRequiredServices(
        [[maybe_unused]] AZ::ComponentDescriptor::DependencyArrayType& required)
    {
        // required.push_back(AZ_CRC("ConversationEditorService", 0x11a7a5d5));
    }

    void DialogueAssetBuilderComponent::GetDependentServices(
